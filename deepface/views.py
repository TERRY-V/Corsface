...
def facetrack(request):
    if request.method == 'POST':
        ...
        for facetrack_record in facetracks_record:
            ...
            if facetrack_record.status >= 0 and facetrack_record.matched_person_id is not None:
                person_matched = {}
...
def getFacetrackInfo(request):
    if request.method == 'POST':
        ...
        if facetrack_info.status >= 0 and facetrack_info.matched_person_id is not None:
            person_matched = {}
        ...
        for retryTimes in range(settings.FACETRACK_PERSON_RETRY_TIMES):
            ...
            if matchResult["result"]["msg"] == 'The transaction is on processing':
                ...
                for match in matches[0:10]:
                    ...
                    person_match['score'] = smoothingAlgorithm(match['score'])
...
def personGroup(request):
    if request.method == 'GET':
        ...
        for person_group in person_groups:
            ...
            group['uuid'] = person_group.group_uuid
...
def cropFace(request):
    if request.method == 'POST':
        ...
        if photo_path is not None and photo_path.startswith('/'):
            relative_photo_path = photo_path[1:]
        ...
        if int(option) == 1:
            ...
            image = Image.open(relative_photo_path)
            crop_image = image.crop((image.size[0] * x, image.size[1] * y, image.size[0] * w + image.size[0] * x, image.size[1] * h + image.size[1] * y))
            crop_image = crop_image.resize((144, 144), Image.ANTIALIAS)
            ...
            photo_base64 = base64.b64encode(open(relative_photo_path, 'rb').read())
            response = deepFaceAPI.cropface(str(photo_base64, 'utf-8'))
            if response['result']['code'] == 0:
                age = response['result']['results']['age']
                glasses = response['result']['results']['glasses']
                sex = response['result']['results']['sex']
                image_base64 = response['result']['results']['img']
                quality = response['result']['results']['quality']
...
#@login_required(login_url='/corsface/login')
@csrf_exempt
def getPersonFacetrack(request):
    if request.method == 'POST':
        person_id = request.POST.get('person_id')
        ...
        facetracks_record = FacetrackInfo.objects.filter(matched_person__person_id = person_id, recognize_id = int(recognize_id))
        if src_ids is not None and len(src_ids):
            src_ids = src_ids.split(',')
            facetracks_record = facetracks_record.filter(src_id__in=src_ids)
        if start_time is not None and len(start_time):
            facetracks_record = facetracks_record.filter(createdate__gte=start_time)
        if end_time is not None and len(end_time):
            facetracks_record = facetracks_record.filter(createdate__lte=end_time)
        if is_processed is not None and len(is_processed):
            is_processed = int(is_processed)
            if is_processed == 1:
                facetracks_record = facetracks_record.filter(status__lt=2, matched_person_id__isnull=False)
            elif is_processed == 2:
                facetracks_record = facetracks_record.filter(status=2)
            else:
                pass
        if sort is not None and len(sort):
            sort = int(sort)
            if sort == 2:
                facetracks_record = facetracks_record.order_by('-id')
            elif sort == 3:
                facetracks_record = facetracks_record.order_by('matched_score')
            elif sort == 4:
                facetracks_record = facetracks_record.order_by('-matched_score')
            else:
                pass
        ...
        for facetrack_record in facetracks_record:
            ...
            camera_info = CameraInfo.objects.get(src_id=facetrack_record.src_id)
            facetrack['camera_name'] = camera_info.camera_name
            facetrack['scene_image'] = '/image?type=2&id=' + facetrack_record.facetrack_id
            facetrack['status'] = facetrack_record.status
            facetrack['matched_score'] = facetrack_record.matched_score
            facetrack['createdate'] = facetrack_record.createdate.strftime("%Y-%m-%d %H:%M:%S")
            facetrack_images = FacetrackImage.objects.filter(facetrack_id=facetrack_record.facetrack_id)
            facetrack['images'] = []
            for facetrack_image in facetrack_images:
                facetrack['images'].append({
                    'path': '/image?type=4&id=' + facetrack_record.facetrack_id + '&fn=' + facetrack_image.image_name,
                    'quality': facetrack_image.image_quality
                })
            facetracks.append(facetrack)
...
def deletePerson(request):
    if request.method == 'POST':
        person_ids = request.POST.get('person_ids')

        for person_id in json.loads(person_ids):
            person_info = PersonInfo.objects.get(person_id=person_id)
...
def deployPerson(request):
    if request.method == 'POST':
        ...
        if int(option) == 1:
            ...
            person_info = PersonInfo.objects.get(person_id = person_id)
            person_deploy_list = PersonDeploy.objects.filter(deployed_person_id = person_info.id, recognize_id = recognize_id)

            sex = person_info.gender
            sex -= 1 if sex == 1 or sex == 2 else -1

            if not person_deploy_list.exists():
                response = deepFaceAPI.createperson_id(person_id, sex, [person_info.group.group_uuid])
                if response['result']['msg'] == 'SUCC':
                    ...
                    for image_url in images:
                        ...
                        fr.close()
                        deepFaceAPI.addimgtoperson(str(image, "utf-8"), person_id)
                    PersonDeploy.objects.create(deployed_person_id = person_info.id, recognize_id = recognize_id, status = 1)
                    context = {'code': 0, 'message': 'success'}
                else:
                    deepFaceAPI.deleteperson(person_id)
                    PersonDeploy.objects.filter(deployed_person_id = person_info.id).update(status = -1)
                    context = {'code': -2, 'message': 'deploy person error, please retry later'}
            else:
                person_deploy = person_deploy_list[0]
                if person_deploy.status == 0 or person_deploy.status == -1:
                    response = deepFaceAPI.createperson_id(person_id, sex, [person_info.group.group_uuid])
                    if response['result']['msg'] == 'SUCC':
                        images = PersonImage.objects.filter(person_id = person_id).values_list("face_image", flat = True)
                        for image_url in images:
                        ...
                        PersonDeploy.objects.filter(deployed_person_id = person_info.id, recognize_id = recognize_id).update(status = 1)
                        context = {'code': 0, 'message': 'success'}
                    else:
                        deepFaceAPI.deleteperson(person_id)
                        PersonDeploy.objects.filter(deployed_person_id = person_info.id).update(status = -1)
                        context = {'code': -3, 'message': 'deploy person error, please retry later'}
                elif person_deploy.status == 1:
                    context = {'code': -4, 'message': 'person already deployed'}
                else:
                    context = {'code': -5, 'message': 'unknown status error'}
        elif int(option) == 2:
            group_id = int(request.POST.get('group_id'))
            recognize_id = int(request.POST.get('recognize_id'))
            
            person_list = PersonInfo.objects.filter(group_id = group_id, isdeleted = 0)
            already_deployed = 0
            newly_deployed = 0
            for person in person_list:
                if not PersonDeploy.objects.filter(deployed_person = person, recognize_id = recognize_id).exists():
                    person_deploy = PersonDeploy(deployed_person = person, recognize_id = recognize_id)
                    person_deploy.save()
                    newly_deployed = newly_deployed + 1
                else:
...
def deployInfo(request):
    if request.method == 'POST':
        ...
        deploy_list = PersonDeploy.objects.filter(deployed_person__person_id = person_id).order_by('-id')
        count = deploy_list.count()
        deploy_list = deploy_list[(page - 1) * number:page * number]

        persons = []
        for person_deploy in deploy_list:
            person = {}
            person['person_id'] = person_deploy.deployed_person.person_id
            person['recognize_id'] = person_deploy.recognize_id
            person['recognize_name'] = person_deploy.recognize.server_name
            person['status'] = person_deploy.status
            person['created_time'] = person_deploy.created_time.strftime("%Y-%m-%d %H:%M:%S")
            person['name'] = person_deploy.deployed_person.name
            person['gender'] = person_deploy.deployed_person.gender
            person['face_image'] = person_deploy.deployed_person.face_image
...
def deploySearch(request):
    if request.method == 'POST':
        ...
        deploy_list = PersonDeploy.objects.filter(deployed_person__isdeleted = 0).order_by('-id')
        if recognize_id is not None and len(recognize_id):
            deploy_list = deploy_list.filter(recognize_id = int(recognize_id))
        if group_id is not None and len(group_id):
            deploy_list = deploy_list.filter(deployed_person__group_id = int(group_id))
        if status is not None and len(status):
            deploy_list = deploy_list.filter(status = int(status))
        if name is not None and len(name):
            deploy_list = deploy_list.filter(deployed_person__name__contains = name)
        if gender is not None and len(gender):
            deploy_list = deploy_list.filter(deployed_person__gender = int(gender))
        if id_card is not None and len(id_card):
            deploy_list = deploy_list.filter(deployed_person__id_card__contains = id_card)
        if start_time is not None:
            deploy_list = deploy_list.filter(createdate__gte=start_time)
        if end_time is not None:
            deploy_list = deploy_list.filter(createdate__lte=end_time)
        count = deploy_list.count()
        deploy_list = deploy_list[(page - 1) * number:page * number]
        ...
        for person_deploy in deploy_list:
            ...
            person['status'] = person_deploy.status
            person['created_time'] = person_deploy.created_time.strftime("%Y-%m-%d %H:%M:%S")
            person['alarm_level'] = person_deploy.deployed_person.alarm_level
            person['name'] = person_deploy.deployed_person.name
            person['gender'] = person_deploy.deployed_person.gender
            person['birthday'] = person_deploy.deployed_person.birthday
            person['nation'] = person_deploy.deployed_person.nation
            person['id_card'] = person_deploy.deployed_person.id_card
            person['family_register'] = person_deploy.deployed_person.family_register
            person['face_image'] = person_deploy.deployed_person.face_image
            person['group_id'] = person_deploy.deployed_person.group_id
            person['group_name'] = person_deploy.deployed_person.group.group_name
            persons.append(person)
...
def deployDelete(request):
    if request.method == 'POST':
        ...
        int_id = PersonInfo.objects.get(person_id = person_id).id
        person = PersonDeploy.objects.get(deployed_person_id = int_id, recognize_id = recognize_id)
        if person.status == 1:
            response = deepFaceAPI.getpersoninfo(person_id)
            if response['result']['msg'] == "Cann't find Person with the ID":
                person.delete()
                context = {'code': 0, 'message': 'success'}
                return HttpResponse(json.dumps(context), content_type = "application/json")
            response2 = deepFaceAPI.deleteperson(person_id)
            if response2['result']['code'] == 0:
                person.delete()
                context = {'code': 0, 'message': 'success'}
            else:
                context = {'code': -2, 'message': 'delete person error'}
        else:
            person.delete()
            context = {'code': 0, 'message': 'success'}
    else:
...
def deploySynchronize(request):
    if request.method == 'POST':
        ...
        response = deepFaceAPI.createperson_id(person_id, sex, [person.group.group_uuid])
        if response['result']['msg'] == 'SUCC':
            images = PersonImage.objects.filter(person_id = person_id).values_list("face_image", flat = True)
            for image_url in images:
                fr = open(image_url[1:], 'rb')
                ...
                fr.close()
                deepFaceAPI.addimgtoperson(str(image, "utf-8"), person_id)
            PersonDeploy.objects.filter(deployed_person_id = person.id, recognize_id = recognize_id).update(status = 1)
            context = {'code': 0, 'message': 'success'}
        else:
            deepFaceAPI.deleteperson(person_id)
            PersonDeploy.objects.filter(deployed_person_id = person.id, recognize_id = recognize_id).update(status = -1)
            context = {'code': -1, 'message': 'synchronize failed'}
    else:
        context = {'code': -1, 'message': 'method not supported'}
    return HttpResponse(json.dumps(context, sort_keys=True), content_type="application/json")
...
def createPersonFromFacetrack(request):
    if request.method == 'POST':
        ...
        person_id = str(uuid.uuid1())
        person_group = PersonGroup.objects.get(id = group_id)

        response = deepFaceAPI.createperson_id(person_id, -1, [person_group.group_uuid])
        if response['result']['code'] == 0:
            ...
            if addResponse['result']['code'] == 0:
                ...
                person_deploy = PersonDeploy(deployed_person = person, recognize = recognizeServer, status = 1)
                person_deploy.save()

                facetrack_info.status = 2
                facetrack_info.matched_person = person
                facetrack_info.save()
...
def facetrackAlarmStatsToday(request):
    if request.method == 'GET':
        today = datetime.datetime.now().strftime('%Y-%m-%d')
        sql = "select person_info.alarm_level, count(*) from facetrack_info, person_info where facetrack_info.createdate >= '%s' and facetrack_info.matched_person_id = person_info.id group by alarm_level" % (
            today)
    ...
    if request.method == 'POST':
        ...
        if int(id) > 0:
            ...
            try:
                ...
                response_data['code'] = -1
                response_data['message'] = '请求超时，请稍后重试'
    return HttpResponse(json.dumps(response_data), content_type="application/json")
...
def recognizeList(request):
    ...
    if request.method == 'POST':
        ...
        if not quest:
            ...
            record_list = RecognizeServer.objects.filter(Q(server_name__icontains=quest) | Q(appkey__icontains=quest))
...
def recognizeDelete(request):
    ...
    if request.method == 'POST':
        id = request.POST.get('id', 0)
        num = RecognizeServer.objects.filter(id=id, is_enabled=1).count()
        if num > 0:
            response_data = {'code': -1, 'message': '请先停用该识别端'}
            return HttpResponse(json.dumps(response_data), content_type="application/json")
        if int(id) > 0:
    ...
    if request.method == 'POST':
        ...
        if int(id) > 0:
            try:
                ...
                if response['result']['code'] == 0 or msg == 'The application is actived now' or msg == "Appkey isn't correct":
                    res.is_enabled = 0 if int(state) == 1 else 1
                    res.save()
                    response_data['code'] = 0
                    response_data['message'] = 'success'
...
def recognizeSync(request):
    ...
    if request.method == 'POST':
        ...
        if int(resultset.is_enabled) == 0:
            response_data = {'code': -1, 'message': '请先启用识别端'}
            return HttpResponse(json.dumps(response_data), content_type="application/json")
        ...
        try:
            ...
            response_data = {'code': -1, 'message': '请求超时，请稍后重试'}
            return HttpResponse(json.dumps(response_data), content_type="application/json")
